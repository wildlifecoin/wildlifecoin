...
uint256 hashGenesisBlock("0xa69eeb8de2406eb73fbf258ee6eee171bef80eae0b882649a1a9e87459f9ecac");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Wildlifecoin: starting difficulty is 1 / 2^12
...
int64 nChainStartTime = 1393215194; // Line: 2815
...
const string strMessageMagic = "Wildlifecoin Signed Message:\n";
...
{
    // Wildlifecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if(nHeight == 1)
    {
        nSubsidy = 2500000 * COIN; // First block is the pre mine of 2.5%
    }
    else if(nHeight < 120)
    {
        nSubsidy = 0; // First 120 blocks give 0 reward to confirm pre-mine
    }
    else if(nHeight < 1000000)
    {
        rand0 = generateMTRandom(seed, 99);
...
static const int64 nTargetTimespan = 4 * 60 * 60; // WildlifeCoin: every 4 hours
static const int64 nTargetSpacing = 2.5 * 60; // WildlifeCoin: block released every 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Wildlifecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
                if (pindexLast->nHeight+1 >= 35) { DiffMode = 2; } // wildlifecoin, 100 blocks after first legacy diff adjustment
        }
        else {         
        	if (pindexLast->nHeight+1 >= 1000) { DiffMode = 2; }  //wildlifecoin, Let early adopters have fun.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Wildlifecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Wildlifecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is WildlifeCoin. 02/23/14 woooooooo";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x18fc637b9f68da72d9983b2ef4be566fe32c642b3cfd6d838c38247c413bade9"));
...
unsigned char pchMessageStart[4] = { 0xfa, 0xbf, 0xb5, 0xda }; // Wildlifecoin: increase each by adding 1 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WildlifecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WildlifecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WildlifecoinMiner(CWallet *pwallet)
{
    printf("WildlifecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("wildlifecoin-miner");
    ...
    try { loop {
        ...
        printf("Running WildlifecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("WildlifecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WildlifecoinMiner, pwallet));
}
